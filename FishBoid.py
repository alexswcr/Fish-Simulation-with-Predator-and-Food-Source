...
class FishBoid():
    def __init__(self, window, colour, grid, foodpoints,evo_and_learn,stochastic):
        self.colour = colour
        self.age = 0
        ...
        self.Hunger = np.random.randint(1500, 2250)
    ...
    def reproduce(self):
        ...
        if isinstance(partner, FishBoid) and partner is not self:
            ...
            baby_colour = np.clip(np.uint8(np.mean((self.colour, partner.colour), 0)) + np.random.randint(-20, 20, 3),
                                  0, 255)
            baby_fish = FishBoid((self.window[1], self.window[0]), baby_colour, self.grid, self.foodpoints,self.evo_and_learn,self.stochastic)
